...
public class GuiPoketamableRename extends Screen
{
	...
	public GuiPoketamableRename(ItemStack stack) 
	{
		...
		maxHealth = (stack.getTag() != null && stack.getOrCreateChildTag("pokemonmd").getFloat("maxhealth") != 0) ? stack.getTag().getCompound("pokemonmd").getFloat("maxhealth") : 0F;
		health = (stack.getTag() != null && stack.getOrCreateChildTag("pokemonmd").getString("health") != "") ? stack.getOrCreateChildTag("pokemonmd").getFloat("health") : 0F;
	...
	public void init()
	{
		...
		textfield.setText(PoketamableSummonItem.getPoketamableNameFromItemStack(poketamableItemStack));
	}
...
/*
		...
		if (!textfield.keyPressed(keyCode, scanCode, modifiers)) 
		{
            ...
            if (keyCode == 257) 
            {
                if (!textfield.getText().equals("")) 
                {
                	KindredLegacy.network.sendToServer(new PoketamableNameMessage(Minecraft.getInstance().player.getName().getString(), textfield.getText()));
                }
