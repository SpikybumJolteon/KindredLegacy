...
public class BlockAlchemenisizer extends BlockBase implements IPylonPowered
{
	...
	public BlockAlchemenisizer()
	{
		super("alchemenisizer", Material.ROCK);
	...
	{
		return Item.getItemFromBlock(KindredLegacyBlocks.ALCHEMENISIZER);
	}

	@Override
	public ItemStack getItem(World worldIn, BlockPos pos, IBlockState state)
	{
		return new ItemStack(KindredLegacyBlocks.ALCHEMENISIZER);
	}
			...
			EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
	{
		if(!worldIn.isRemote)
		{
			playerIn.openGui(KindredLegacyMain.instance, ModInfo.GUI_ALCHEMENISIZER, worldIn, pos.getX(), pos.getY(), pos.getZ());
		}
	...
	public static void setState(boolean active, World worldIn, BlockPos pos) 
	{
		...
		if(active) worldIn.setBlockState(pos, KindredLegacyBlocks.ALCHEMENISIZER.getDefaultState().withProperty(FACING, state.getValue(FACING)).withProperty(ACTIVE, true), 3);
		else worldIn.setBlockState(pos, KindredLegacyBlocks.ALCHEMENISIZER.getDefaultState().withProperty(FACING, state.getValue(FACING)).withProperty(ACTIVE, false), 3);
	...
	{
		return new TileEntityAlchemenisizer();
	}
	...
	{
		TileEntityAlchemenisizer tileEntity = (TileEntityAlchemenisizer)worldIn.getTileEntity(pos);
	...
	{
		if(worldIn.getTileEntity(pos) instanceof TileEntityAlchemenisizer)
		{
			TileEntityAlchemenisizer tileentity = (TileEntityAlchemenisizer)worldIn.getTileEntity(pos);
