package fuzzyacornindustries.kindredlegacy.client.renderer.entity.model;

import com.google.common.collect.ImmutableList;

import fuzzyacornindustries.kindredlegacy.KindredLegacy;
import fuzzyacornindustries.kindredlegacy.client.animation.AnimationDeployer;
...
import fuzzyacornindustries.kindredlegacy.client.animation.JointAnimation;
import fuzzyacornindustries.kindredlegacy.client.animation.PartAnimate;
import fuzzyacornindustries.kindredlegacy.client.animation.PartInfo;
...
import fuzzyacornindustries.kindredlegacy.lib.action.LibraryUniversalAttackID;
import net.minecraft.client.renderer.entity.model.TintedAgeableModel;
import net.minecraft.client.renderer.model.ModelRenderer;
import net.minecraft.entity.Entity;
import net.minecraft.util.math.MathHelper;

public class AhriNinetalesModel<T extends TamablePokemon> extends TintedAgeableModel<T>
{
	...
	@Override
	public void setRotationAngles(T entity, float distanceMoved, float horzVelocity, float ageInTicks, float netHeadYaw, float headPitch)
	{
		animate(entity, distanceMoved, horzVelocity, ageInTicks, netHeadYaw, headPitch);
	}
	...
	public void animate(Entity entity, float distanceMoved, float horzVelocity, float ageInTicks, float netHeadYaw, float headPitch)
	{	
		...
		float gravityFactor = (float)((IGravityTracker)entity).getGravityFactor();

		float idleDampener = 1F;
		boolean isSitting = false;

		float vastayaNinetalesHealthPercent = 1F;

		if(entity instanceof VastayaNinetalesEntity)
		{
			isSitting = ((VastayaNinetalesEntity) entity).isSitting();

			if(((VastayaNinetalesEntity) entity).isTamed())
			{
				vastayaNinetalesHealthPercent = ((VastayaNinetalesEntity) entity).getHealthPercent();
			}
		}
		else
		{
		...
		float tick = animationDeployer.getEntity().getAnimationTick() + KindredLegacy.proxy.getPartialTick();
		...
		{
			idleDampener = animateFoxfireSummon(animationDeployer.getEntity(), distanceMoved, horzVelocity, ageInTicks, netHeadYaw, headPitch);
		}
		else if(animationDeployer.getEntity().getAnimationID() == LibraryAhriNinetalesAttackID.FIREBALL && !isSitting)
		{
			idleDampener = animateFireballAttack(animationDeployer.getEntity(), distanceMoved, horzVelocity, ageInTicks, netHeadYaw, headPitch);
		}
				...
				(animationDeployer.getEntity().getAnimationID() == LibraryAhriNinetalesAttackID.FIREBLAST && tick > 28))) && !isSitting)
		{
			...
			idleDampener = animateJumpFireballAttack(animationDeployer.getEntity(), distanceMoved, horzVelocity, ageInTicks, netHeadYaw, headPitch, tick);
		}
		else if(animationDeployer.getEntity().getAnimationID() == LibraryUniversalAttackID.IGNITE && !isSitting)
		{
			idleDampener = animateIgnite(animationDeployer.getEntity(), distanceMoved, horzVelocity, ageInTicks, netHeadYaw, headPitch);
		}
...
//		if(entity instanceof VastayaNinetalesEntity)
//		{
//			sizeModifier = ((VastayaNinetalesEntity) entity).defaultHeight / entity.height;
//		}
...
//		}
		...
		animateBody((IAnimateAhriNinetales)entity, distanceMoved, horzVelocity, ageInTicks, netHeadYaw, headPitch, sizeModifier, idleDampener, isSitting, verticalVelocity, gravityFactor);
		animateHead((IAnimateAhriNinetales)entity, distanceMoved, horzVelocity, ageInTicks, netHeadYaw, headPitch, sizeModifier, idleDampener, isSitting, vastayaNinetalesHealthPercent, verticalVelocity, gravityFactor);
		animateArms((IAnimateAhriNinetales)entity, distanceMoved, horzVelocity, ageInTicks, netHeadYaw, headPitch, sizeModifier, idleDampener, isSitting, verticalVelocity, gravityFactor);
		animateLegs((IAnimateAhriNinetales)entity, distanceMoved, horzVelocity, ageInTicks, netHeadYaw, headPitch, sizeModifier, idleDampener, isSitting, verticalVelocity, gravityFactor);
		animateTail((IAnimateAhriNinetales)entity, distanceMoved, horzVelocity, ageInTicks, netHeadYaw, headPitch, sizeModifier, idleDampener, isSitting, angularVelocity, verticalVelocity);
	...
	public float animateFoxfireSummon(IAnimatedEntity entity, float distanceMoved, float horzVelocity, float ageInTicks, float netHeadYaw, float headPitch)
	{
		float idleDampener = 1F;
	...
	public float animateJumpFireballAttack(IAnimatedEntity entity, float distanceMoved, float horzVelocity, float ageInTicks, float netHeadYaw, float headPitch, float tick)
	{
		float idleDampener = 1F;
	...
	public float animateFireballAttack(IAnimatedEntity entity, float distanceMoved, float horzVelocity, float ageInTicks, float netHeadYaw, float headPitch)
	{
		float idleDampener = 1F;
	...
	public float animateIgnite(IAnimatedEntity entity, float distanceMoved, float horzVelocity, float ageInTicks, float netHeadYaw, float headPitch)
	{
		float idleDampener = 1F;
	...
	public void animateBody(IAnimateAhriNinetales entity, float distanceMoved, float horzVelocity, 
			float ageInTicks, float netHeadYaw, float headPitch, float modelSize, float idleDampener, boolean isSitting, float verticalVelocity, float gravityFactor)
	{
		if(!isSitting)
		{
	...
	public void animateHead(IAnimateAhriNinetales entity, float distanceMoved, float horzVelocity, 
			float ageInTicks, float netHeadYaw, float headPitch, float modelSize, float idleDampener, boolean isSitting, float vastayaNinetalesHealthPercent, float verticalVelocity, float gravityFactor)
	{
		JointAnimation.reverseJointRotatesChange(bodyInfo, headJointInfo);

		PartAnimate.headAnimateInfoOnlyWithAngleModifiers(headInfo, netHeadYaw, headPitch, 0.9F * idleDampener, 0.9F * idleDampener);
		...
		earLftInfo.setNewRotateZ(earLftInfo.getNewRotateZ() - (1F - vastayaNinetalesHealthPercent) * healthChangeAngle);
		earRtInfo.setNewRotateZ(earRtInfo.getNewRotateZ() + (1F - vastayaNinetalesHealthPercent) * healthChangeAngle);
	}
	...
	public void animateTail(IAnimateAhriNinetales entity, float distanceMoved, float horzVelocity, 
			float ageInTicks, float netHeadYaw, float headPitch, float modelSize, float idleDampener, boolean isSitting, float angularVelocity, float verticalVelocity)
	{
	...
	{
		return ImmutableList.of(this.bodyJoint);
	}
}