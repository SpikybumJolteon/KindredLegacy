		...
		else
		{
			for (int l = 0; l < 3; ++l)
			{
				...
				if (!this.world.isRemote && numberOfMinionsToSpawn != 0)
				{
						...
						{
							ClaySkittyEntity entityMinion = new ClaySkittyEntity(KindredLegacyEntities.CLAY_SKITTY, this.world);

							spawnMinion(entityMinion, i1, j1, k1);
						}
						else if (randomInt >= 7 && randomInt <= 9)
						{
							ClayPurrloinEntity entityMinion = new ClayPurrloinEntity(KindredLegacyEntities.CLAY_PURRLOIN, this.world);

							spawnMinion(entityMinion, i1, j1, k1);
						}
						else if (randomInt == 10)
						{
							ClayEspurrEntity entityMinion = new ClayEspurrEntity(KindredLegacyEntities.CLAY_ESPURR, this.world);
