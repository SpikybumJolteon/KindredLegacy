...
import fuzzyacornindustries.kindredlegacy.entity.mob.ai.MuBunearyChannelConfuseGoal;
import fuzzyacornindustries.kindredlegacy.entity.mob.ai.MuBunearySpinAttackGoal;
import fuzzyacornindustries.kindredlegacy.lists.KindredLegacyParticles;
...
public class MuBunearyEntity extends HostilePokemon implements IAnimatedEntity, IRangedAttackMob
{
	...
	public void attackEntityWithRangedAttack(LivingEntity targetEntity, float par2)
	{
		if(this.getAnimationID() == LibraryOkamiPokemonAttackID.NO_ACTION)
		{
			...
			{
				KindredLegacy.network.send(PacketDistributor.TRACKING_ENTITY.with(() -> this), new ActionAnimationMessage((byte)MuBunearyEntity.actionIDCastConfuse, this.getEntityId()));
			...
			{
				KindredLegacy.network.send(PacketDistributor.TRACKING_ENTITY.with(() -> this), new ActionAnimationMessage((byte)MuBunearyEntity.actionIDSpinAttack, this.getEntityId()));
	...
	public void spawnConfusionParticle()
	{
		...
		double xPos = this.getPosX() + (this.world.rand.nextFloat() - 0.5D) * this.getWidth() * 1D;
		double yPos = this.getPosY() + this.world.rand.nextFloat() * this.getHeight() - 0.5F;
		double zPos = this.getPosZ() + (this.world.rand.nextFloat() - 0.5D) * this.getWidth() * 1D;

		this.getEntityWorld().addParticle(KindredLegacyParticles.CONFUSE_PARTICLE, xPos, yPos, zPos, motionX, motionY, motionZ);
	}
