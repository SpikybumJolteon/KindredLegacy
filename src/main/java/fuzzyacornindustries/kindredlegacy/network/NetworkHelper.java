 ...
 * 
 * @author AtomicStryker, original author
 *
 */
public class NetworkHelper
{
     ...
     * @param targetPoint
     */
    public void sendPacketToAllAroundPoint(IPacket packet, TargetPoint targetPoint)
    {
        checkClassAndSync(packet.getClass());
        serverOutboundChannel.attr(FMLOutboundHandler.FML_MESSAGETARGET).set(FMLOutboundHandler.OutboundTarget.ALLAROUNDPOINT);
        serverOutboundChannel.attr(FMLOutboundHandler.FML_MESSAGETARGETARGS).set(targetPoint);
        serverOutboundChannel.writeOutbound(packet);
        isCurrentlySendingSemaphor = false;
    }
    ...
    /**
     * Since the crash that happens if we don't do this is complete garbage
     */
